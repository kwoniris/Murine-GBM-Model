...
### Plotting spots of gene expression without the tumor ### 
  ggplot(data, aes(x = x, y = y, col=log10(rowSums(gexp)+1))) + 
    geom_point(size = 0.1) + scale_color_gradient(low = 'lightgrey', high='red') + 
    facet_wrap(~dataset, scales="free", ncol = 2) + theme_classic()
  
  ## Filter out tumor regions with those greater than 3 
  testspots <- log10(rowSums(gexp)+1)>2.5
  ## Getting those spots 
  truespots <- names(which(testspots))
  ## Plot those spots without the tumor region 
  ggplot(data[testspots,], aes(x = x, y = y)) + geom_point(size = 0.1, 
